#' pois_local_vcm_cv: Cross-Validation for Poisson Time-Varying Coefficient Model 
#'
...
#'
#' @return pois_local_vcm_cv returns a list containing the results from the cross-validation
#' \itemize{
...
#' @examples 
#' 
#' cv_fit <- pois_local_vcm_cv(formula = Y~x1+x2, data = pois_data, time = time, id = id)
#' 
#' summary(cv_fit)
#' 
pois_local_vcm_cv <- function(formula, data, time, id = NULL,
  ...
  for (bb in 1:length(hh)){
    ...
    if (is.null(data$id)) {
        ...
        for (cv in 1:id_max){
          ...
          cvrvcm[1,] <- optim(ivcm, pois_local_loglik, 
                              gr = gr_pois_local_loglik,
                              x = cvmm, y = cvmr, time = cvmt,
          ...
          for (ee in 2:length(cviimt)){
            ...
            grvcm <- gr_pois_local_loglik(cvmm, ovcm, cvmr, cvmt,
                                          as.numeric(cv_df$time[ee]), 
                                          hhh, kernel_id)
            hsvcm <- hs_pois_local_loglik(cvmm, ovcm, cvmr, cvmt,
                                          as.numeric(cv_df$time[ee]), 
                                          hhh, kernel_id)
            cvrvcm[ee,] <- ovcm - solve(hsvcm) %*% grvcm
          }
          cv_loglik[cv] <- pois_cv_local_loglik(cviimm, as.matrix(cvrvcm), cviimr, cviimt)
        }
        pre_results <-sum(cv_loglik) 
      }
