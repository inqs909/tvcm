poisson_vcm_cv <- function(formula, data, time, id = NULL,
                           method = "local-linear",
                           local_params = NULL,
  ...
  if (method == "local-linear"){
    if (is.null(local_params)){
      results <- poisson_local_vcm_cv(formula = formula, data = data,
                                      time = time, id = id)
      return(results)
    } else {
      ...
      results <- poisson_local_vcm_cv(formula = formula, data = data, time = time, id = id,
