#' binary__local_vcm_cv: Cross-Validation for Binary Time-Varying Coefficient Model 
#'
...
#'
#' @return binary_local_vcm_cv returns a list containing the results from the cross-validation
#' \itemize{
...
#' @examples 
#' 
#' cv_fit <- binary_vcm_cv(formula = Y~x1+x2, data = binary_data, time = time, id = id)
#' 
#' summary(cv_fit)
#' 
binary_local_vcm_cv <- function(formula, data, time, id = NULL,
  ...
  for (bb in 1:length(hh)){
    ...
    if (is.null(data$id)) {
        ...
        for (cv in 1:id_max){
          ...
          nvcm <- optim(ivcm, bin_logit_local_loglik, 
                        gr = gr_bin_logit_local_loglik,
                        x = cvmm, y = cvmr, time = cvmt,
                        time_zero = as.numeric(cviimt),
                        h = hhh, type = kernel_id, method = "BFGS")$par
          cv_loglik[cv] <- bin_cv_loglik(cviimm, as.matrix(nvcm), cviimr, cviimt)
        }
        ...
        for (cv in 1:id_max){
          ...
          cvrvcm[1,] <- optim(ivcm, bin_logit_local_loglik,
                              gr = gr_bin_logit_local_loglik,
                              x = cvmm, y = cvmr, time = cvmt,
                              time_zero = as.numeric(cviimt[1]),
                              h = hhh, type = kernel_id, method = "BFGS")$par
          for (ee in 2:length(cviimt)){
            ovcm <- as.vector(cvrvcm[(ee-1),])
            grvcm <- gr_bin_logit_local_loglik(cvmm, ovcm, cvmr, cvmt,
                                         as.numeric(cv_df$time[ee]),
                                         hhh, kernel_id)
            hsvcm <- hs_bin_logit_local_loglik(cvmm, ovcm, cvmr, cvmt,
                                         as.numeric(cv_df$time[ee]),
                                         hhh, kernel_id)
            cvrvcm[ee,] <- ovcm - solve(hsvcm) %*% grvcm
          }
          cv_loglik[cv] <- bin_cv_local_loglik(cviimm, as.matrix(cvrvcm),
                                               cviimr, cviimt)
        }
        pre_results <-sum(cv_loglik) 
      }
